...
public class LengthCharachter {
    ...
    static char[] f(char[] a, int start, int len){
        if(len > a.length || start+len > a.length || start<0 || len<0)  return null;
        char[] returnedArray = new char[len];
        int returnedArrayIndex = 0;
            for(int i=start; i< start + len; i++){
                returnedArray[returnedArrayIndex] = a[i];
                returnedArrayIndex++;
            }
